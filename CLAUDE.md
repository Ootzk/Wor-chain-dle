...
PUBLIC_URL=/ npm run build # 프로덕션 빌드 (로컬 테스트용, CI에서는 불필요)
npm test               # 단위 테스트
...
`Date` 대신 `Temporal` API (`temporal-polyfill`)를 전면 사용. 앱 런타임에 `Date` 객체 없음. 단어 선택 · subtitle · 달력 · 공유 텍스트 · 카운트다운 모두 `Temporal.Now.plainDateISO()` 기반. Playwright의 `page.clock.setFixedTime()`이 `Temporal.Now`에도 영향을 미침.
...
- `local-timezone.spec.ts` — 로컬 타임존 단어 리셋, dailyHistory 마이그레이션 (`page.clock.setFixedTime` + `timezoneId` 활용)
...
- **날짜 고정**: `page.clock.setFixedTime()`으로 2026-02-20 고정 → `hydro`가 데일리 단어가 되어 Daily 모드에서 직접 플레이. Custom 모드 해킹 없이 자연스러운 Daily 스크린샷 촬영. (`temporal-polyfill`의 `Temporal.Now`도 Playwright clock에 영향받음 — `e2e/local-timezone.spec.ts`에서 검증 완료)
- **hydro 날짜 유래**: `WORDS[4]` = "hydro", epoch(2026-02-16) + 4일 = 2026-02-20
- **Calendar 별도 날짜**: Calendar 스크린샷만 2026-03-12로 별도 설정 (더 많은 history 데이터 필요)

## Header Icons

모드별 헤더 아이콘 구성:

| 아이콘 | Daily | Practice | Custom | Create |
| --- | --- | --- | --- | --- |
| Info (ⓘ) | O | O | O | O |
| Stats (📊) | O | X | X | X |
| Settings (⚙️) | O | O | O | O |
...
Stats 아이콘은 Daily 모드 전용 (`isDaily && ...`). 스크린샷 스크립트에서 아이콘 인덱스: Info=0, Stats=1, Settings=2, Donate=3 (Daily), Info=0, Settings=1, Donate=2 (기타).
...
번역 리소스를 JS 번들에 인라인 (`src/i18n.ts`). `src/locales/{lang}/translation.json`이 빌드 시 포함됨. HTTP 백엔드 미사용. 언어 감지: localStorage `i18nextLng` → `CONFIG.defaultLang`.

## Snake Chain Rule

2번째 추측부터 이전 추측과 체인으로 연결. 홀수→짝수: 끝 글자 체인, 짝수→홀수: 첫 글자 체인 (1-indexed). 검증: `App.tsx`의 `onEnter()`.

## Version History

- **v0.x** — 포크 초기 세팅, 영어 Wordle 기본 구현, QWERTY 키보드.
- **v1.0.x** — Snake chain rule, ChainBridge 시각화, dead end, Share 포맷(box-drawing), GitHub Pages 배포, GoatCounter 연동.
